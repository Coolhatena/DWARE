...
# os.chdir('/home/mn/dware')
db_dir = './webconfig/db/dware.json'
...
# out = cv.VideoWriter('output21.mp4', fourcc, 30.0, (img_width, img_height))
...
def update_user_name():
    global current_user, current_user_id 
    with open(db_dir) as db:
        db_data = json.load(db)
        user_id = db_data['current'][0]['userID']
        users = db_data['users']
        for user in users:
            print(user['id'])
            print(user_id)
...
def sendIncidentPOST(description):
    global current_user_id
    now = datetime.now()
    date_string = now.strftime("%d/%m/%Y %H:%M:%S")
    apiUrl = 'http://10.42.0.1:3000/incidents'
    data = {
...
last_modified_db_time = get_time(db_dir)

with map_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
    # starting time here
    start_time = time.time()
    eyes_closed_time_start = time.time()
    eyes_closed_time = 0
    is_eyes_closed_last_frame = False
    is_eyes_closed_actual = False
    
    # starting Video loop here.
    while True:
        # led_start.on()
        if last_modified_db_time != get_time(db_dir):
            update_user_name()
            last_modified_db_time = get_time(db_dir)
        ...
        try:
            if results.multi_face_landmarks:
                ...
                if ratio > 3.7:
                    ...
                    if is_eyes_closed_actual:
                       is_eyes_closed_last_frame = True
                    else:
                      is_eyes_closed_actual = True
                      eyes_closed_time_start = time.time()
                      
                    if is_eyes_closed_last_frame:
                        eyes_closed_time = time.time() - eyes_closed_time_start
                        # if(eyes_closed_time > 2):
                            # led.on()
                            # buzzer.on()
                        print(eyes_closed_time)
                    ...
                    if CEF_COUNTER > CLOSED_EYES_FRAME:
                        ...
                        is_eyes_closed_last_frame = False
                        if(eyes_closed_time > 2):
                            sendIncidentPOST("Signos de cansancio")
                        eyes_closed_time = 0
                        # led.off()
                        # buzzer.off()
                        TOTAL_BLINKS += 1
                ...
                utils.colorBackgroundText(
                    frame, f'R: {eye_position_right}', FONTS, 1.0, (40, 220), 2, color[0], color[1], 8, 8)
            ...
            print("Eyes out of bounds")
            print(e)

        frame = utils.textWithBackground(
            frame, f'User: {current_user}', FONTS, 1.0, (30, 50), bgOpacity=0.9, textThickness=2)
